...
#'
#' @export
pmlkhfix <- function(dat, group, fixstruct, niter = 50, eps = 1e-2, nlag = 20,plot=TRUE,
                     nburnin = 5e3, nsamp = 1e5, nintv = 5e1, maxcyc = 2,
  ...
  if(plot==TRUE){
    paramhistory=array(fit[[19]], c(niter, np, np))
    draw(paramhistory)
  }
